...
void PictureInPictureManager::enablePiPForAllVideos(WebView *webView) {
  ...
  QString enableScript = R"(
        (function() {
            console.log('すべての動画をPicture-in-Picture対応に設定中...');

            // 全ての動画要素をPiP対応にする関数
            function enablePiPForAllVideos() {
                ...
                videos.forEach((video, index) => {
                    ...
                    ['disablepictureinpicture', 'disablePictureInPicture', 'disable-picture-in-picture'].forEach(attr => {
                    ...
                    try {
                        ...
                        console.log(`動画 ${index + 1}: disablePictureInPicture プロパティを false に固定しました`);
                    } catch (e) {
                        video.disablePictureInPicture = false;
                        console.log(`動画 ${index + 1}: disablePictureInPicture プロパティを false に設定しました`);
                    }
                    ...
                    video.setAttribute = function(name, value) {
                        if (name.toLowerCase().includes('disablepictureinpicture') ||
            ...
            function setupVideoObserver() {
                ...
                observer.observe(document.body, {
                    ...
                    subtree: true,
                    attributes: true,
                    attributeFilter: ['disablepictureinpicture', 'disable-picture-in-picture']
                });
            ...
            try {
                ...
                console.log('HTMLVideoElement.prototype.disablePictureInPicture を false に固定しました');
            } catch (e) {
...
QString PictureInPictureManager::generatePiPJavaScript() const {
  return R"(
        (async function() {
            console.log('=== 強化PiP実装開始 ===');

            // Step 1: 完全な環境診断
            console.log('🔍 環境診断開始...');
            console.log('User Agent:', navigator.userAgent);
            console.log('Platform:', navigator.platform);
            console.log('WebKit版本:', navigator.appVersion);
